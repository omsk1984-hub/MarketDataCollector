...
public class MarketDataProcessorTests
{
    ...
    public async Task ProcessBatchAsync_SavesNewTicksToRepository()
    {
        ...
        _repositoryMock.Verify(x => x.BulkInsertFastAsync(
            It.IsAny<IReadOnlyList<TickData>>(), It.IsAny<ITimeService>(), It.IsAny<CancellationToken>()), Times.AtLeastOnce);
    }
    ...
    public async Task ProcessBatchAsync_WhenRepositoryThrows_LogsErrorAndConsumerContinues()
    {
        ...
        _repositoryMock
            .Setup(x => x.BulkInsertFastAsync(It.IsAny<IReadOnlyList<TickData>>(), It.IsAny<ITimeService>(), It.IsAny<CancellationToken>()))
            .Returns<IReadOnlyList<TickData>, ITimeService, CancellationToken>((ticks, ts, ct) =>
            {
    ...
    public async Task ProcessBatchAsync_SingleReader_ProcessesBatchesSequentially()
    {
        ...
        _repositoryMock
            .Setup(x => x.BulkInsertFastAsync(It.IsAny<IReadOnlyList<TickData>>(), It.IsAny<ITimeService>(), It.IsAny<CancellationToken>()))
            .Returns<IReadOnlyList<TickData>, ITimeService, CancellationToken>(async (ticks, ts, ct) =>
            {
        ...
        // BulkInsertFastAsync вызывается последовательно
    ...
    public async Task ProcessBatchAsync_DbException_ContinuesProcessing()
    {
        ...
        // Проверяем, что BulkInsertFastAsync вызывался минимум 3 раза
    ...
    public async Task FlushTimer_FlushesPartialBatch_OnTimerTick()
    {
        ...
        _repositoryMock
            ...
            .Returns<IReadOnlyList<TickData>, ITimeService, CancellationToken>((ticks, ts, ct) =>
            {
                _output.WriteLine($"BulkInsertFastAsync called with {ticks.Count} ticks");
                return Task.FromResult(ticks.Count);
            });
        ...
        // BulkInsertFastAsync должен быть вызван минимум 1 раз (таймерный сброс) + финальный flush (пустой после таймера)
        _repositoryMock.Verify(
            x => x.BulkInsertFastAsync(It.IsAny<IReadOnlyList<TickData>>(), It.IsAny<ITimeService>(), It.IsAny<CancellationToken>()),
            Times.AtLeastOnce);
    }
    ...
    public async Task FlushTimer_MultipleTimerTicks_OnlyOneFlush()
    {
        ...
        _repositoryMock
            ...
            .Returns<IReadOnlyList<TickData>, ITimeService, CancellationToken>((ticks, ts, ct) =>
            {
                ...
                _output.WriteLine($"BulkInsertFastAsync call #{callCount} with {ticks.Count} ticks");
                return Task.FromResult(ticks.Count);
            });
        ...
        // Должно быть минимум 2 вызова BulkInsertFastAsync:
    ...
    public async Task ProcessBatchAsync_WithDedupCache_SkipsDuplicateTicks()
    {
        ...
        // Второй батч: тик отфильтрован кэшем → 0 entities → BulkInsertFastAsync вернул 0.
        ...
        // Assert — BulkInsertFastAsync вызывается дважды (по разу на каждый батч),
        // но во втором вызове 0 entities (кэш отфильтровал дубликат).
        insertedCounts.Should().HaveCount(2, "BulkInsertFastAsync вызывается для каждого батча");
