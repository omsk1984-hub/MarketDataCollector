...
public class MarketDataProcessorTests
{
    ...
    public async Task ProcessBatchAsync_SavesNewTicksToRepository()
    {
        ...
        // Arrange
        _repositoryMock
            .Setup(x => x.BulkCopyAsync(It.IsAny<IReadOnlyList<TickData>>(), It.IsAny<ITimeService>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(2);
    ...
    public async Task ProcessBatchAsync_WhenRepositoryThrows_LogsErrorAndConsumerContinues()
    {
        ...
        _repositoryMock.Verify(
            x => x.BulkCopyAsync(It.IsAny<IReadOnlyList<TickData>>(), It.IsAny<ITimeService>(), It.IsAny<CancellationToken>()),
            Times.AtLeast(2),
