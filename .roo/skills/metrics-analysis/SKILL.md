...
7. **ВСЕГДА проверяй содержимое `traces/` командой, а не индексацией файлов.** 
Папка `traces/` содержит большие бинарные файлы (`*.nettrace`, `*.gcdump` по 5–20+ MB, speedscope.json), из-за чего `list_files`/семантический поиск по ней **не работает** (индексация не проходит) и может ошибочно показать папку пустой. Это НЕ значит, что данных нет — они есть, просто не индексируются. Единственный достоверный способ узнать состав `traces/` — выполнить команду `powershell -NoProfile -Command "Get-ChildItem -Path traces | Select-Object Name, Length, LastWriteTime | Sort-Object LastWriteTime -Descending | Format-Table -AutoSize | Out-String -Width 200"` (см. Шаг 1).
