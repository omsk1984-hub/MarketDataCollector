...
Ниже — планы по расширению проекта, опирающиеся на системный дизайн в
[`SD/system-design-interview.md`](SD/system-design-interview.md) (раздел «Что бы я сделал дальше»).
Каждый пункт — самостоятельная вертикаль, реализуется отдельным планом в [`plans/`](plans/).

### P0 — Горизонтальное масштабирование (через Kafka)

- **Разнести Workers по тикер-шардам** — несколько инстансов Worker, каждый подписан на свой
  набор тикеров; балансировка подписки.
- **Kafka как полноценный транспорт сырых тиков:** подключить буферизацию `raw-ticks` между
  Processor и Async Writer (сейчас Kafka используется для `aggregated-data`/`connection-events`,
  но не для сырых тиков). Это развяжет входящие пики и консьюминг, а также позволит нескольким
  Worker'ам и потребителям делить один поток.
- **Per-node idle-статус** через `/health` + Prometheus для оркестрации шардов.

### P1 — Масштабирование хранилища

- **Шардирование БД (Citus)** или кластер PostgreSQL после исчерпания возможностей
  партиционирования по дням (текущий объём ~2 млрд тиков/день → retention-партиции
  уже внедрены через `PartitionMaintenanceService`).
- **Дальше**: read-replica для аналитики/отчётов, отделение read-path от write-path.

### P1 — Стриминговая обработка до записи

- **Фильтры/обогащение потока** в стиле Kafka Streams: нормализация, валидация,
  обогащение (например, ликвидность/спред), применение правил до записи в БД.
- **Ретрополнительная докачка**: обработка истории за прошлые окна (backfill).

### P1 — Усиление SLO и реактивности

- **Автоматические алёрты** на `ticks.dropped`, `processor.channel.backlog`,
  `ticks.batch.write.duration` — Prometheus rules + Alertmanager уже в [`docker/`](docker/),
  нужно настроить пороги и нотификации.
...
- Отдельный **Query Service** / API для чтения тиков и свечей (сейчас read-path через
  TickWriteBenchmark; выделить продуктивный read API).
- Свёточный/тиковый кэш для низколатентных запросов котировок.
